# NAACUS Website — Feedback Implementation Status

> **Who this is for:** Board Members, Ministry Leads, Community Leaders  
> **Technical version:** See `FEEDBACK_TRACKER_TECHNICAL.md`  
> **Last Updated:** July 15, 2026

---

## How to Read This Document

| Symbol | Meaning |
|--------|---------|
| ✅ | **Done** — live on the test site |
| 🔒 | **Waiting on you** — we need content or info before we can finish this |
| ⬜ | **Planned** — will be done in a future update |

---

## What Has Been Fixed ✅

### Website Look & Feel
- ✅ The website colors now reflect the **NAACUS logo** — royal blue, leafy green, and gold tones are used throughout the site, including the hero banner.

### Programs & Activities Page
- ✅ The word **"Biannual"** has been corrected to **"Biennial"** to match proper Catholic conference terminology.

### Membership Page
- ✅ The membership form now shows **"(Arch)Diocese"** instead of just "Diocese."
- ✅ A new **"Community / African Catholic Group"** field has been added to the form (e.g., Nigerian Catholic Community, Cameroon Catholic Community).
- ✅ The **"Years in the United States"** field is now clearly marked as **Optional** — no one is required to fill it in.
- ✅ **Membership dues are now clearly shown** at the top of the membership form before anyone fills it out:
  - Individual: **$20.00 per person**
  - Group (2–100 members): **$200.00 one-time**
  - Group (100+ members): **$300.00 one-time**

### 2025 Conference Page
- ✅ A **"View Photo Gallery"** button has been added linking to the official NAACUS 2025 conference photos by photographer Kwame Frimpong (sikapalens.com).
- ✅ The conference location correctly shows **Columbus, OH**.

### Fellowship & Ministries Page
- ✅ **Media Ministry** has been added, with Mary Monney listed as coordinator (mary.monney@naacus.org).
- ✅ **Fundraising Ministry** has been added, with Stella Bello listed as coordinator (stella.bello@naacus.org).

### Leadership Page
- ✅ **Maame** has been removed as Young Adults Ministry Coordinator. The position is now listed as **Open**.
- ✅ **Juliet Njoku** is confirmed as Youth Ministry Coordinator with her full bio already on the site. Her photo just needs to be uploaded.
- ✅ **Stella Bello** has been added as Fundraising Coordinator.
- ✅ **Mary Monney** has been added as Media Coordinator.
- ✅ Faustina (Marketing Coordinator) — confirmed she was never listed on the site; no action was needed.

---

## What We Are Waiting on You For 🔒

The following items are **ready to implement the moment we receive the content.** Please send to Celestin (celestin.mbuyamba@naacus.org) or share via the NAACUS shared drive.

### Leadership Page

| What we need | Who is responsible | Why it matters |
|---|---|---|
| **Updated board members list** | Sally Stovall | Some board members may have changed; the site needs to reflect the current team |
| **Photo of Juliet Njoku** | Juliet Njoku | Her bio is already on the site — just missing her photo |
| **Photo + short bio of Stella Bello** | Stella Bello | She is listed as Fundraising Coordinator but her profile is incomplete |
| **Photo + short bio of Mary Monney** | Mary Monney | She is listed as Media Coordinator but her profile is incomplete |
| **Decision: which board members to hide** | Sally Stovall / President | Some members have no photo or bio — do we hide them or show them with just their name and title? |

### Home Page — Testimonials ("What Our Members Say")

| What we need | Who is responsible | Why it matters |
|---|---|---|
| **New photo for Dr. Obii Aguocha** | Dr. Obii Aguocha | The current placeholder photo is inconsistent with others |
| **New photo for Mrs. Gertrude Udoutun** | Mrs. Gertrude Udoutun | Same reason |
| **Reflections + photos from Young Adults** | Young Adults Ministry | Sally confirmed they submitted — please re-share with Celestin |

### Events Page — NAACUS 2027

| What we need | Who is responsible | Why it matters |
|---|---|---|
| **2027 conference dates and location** | Sally Stovall / Leadership | The Save the Date flyer has the exact dates — please share the flyer or the details |

---

## Planned for a Future Update ⬜

These are lower-priority improvements that will be added over time. No action needed from the board right now.

| Item | Description |
|------|-------------|
| Interactive calendar events | Make event listings clickable with registration links |
| Resources & Links expansion | Add contact info for Women, Youth, Men's, Religious leaders, suicide prevention resources, and prayer apps |
| NAACUS YouTube channel link | Add the YouTube channel to the website — please share the channel URL when ready |
| Spiritual Director contact | Surface Rev. Fr. Benoit Mukamba's contact info in the Resources or Contact section |
| Training Programs section | Add a training programs listing under the Leadership page (per Tess's suggestion) |
| Media & Fundraising ministries — full profiles | Add bios and photos once received from Mary Monney and Stella Bello |

---

## Summary Dashboard

| Category | Total | Done ✅ | Waiting 🔒 | Planned ⬜ |
|----------|-------|---------|-----------|-----------|
| High Priority | 9 | 7 | 2 | 0 |
| Medium Priority | 4 | 3 | 1 | 0 |
| Low Priority | 5 | 1 | 1 | 3 |
| **Total** | **18** | **11** | **4** | **3** |

---

*Questions? Contact Celestin Mbuyamba — celestin.mbuyamba@naacus.org*
